...
/**
 * Headless Social Trend Intelligence Pipeline (Identify -> Analyse -> Act)
 * 
 * Channels: Instagram, TikTok, Facebook, Influencers
 * Brain: Google Gemini 2.5 Flash / Backboard AI
 * 
 * Usage:
 *   node scripts/trend_pipeline.mjs --topic "Crispy Falafel" --brain gemini
 *   node scripts/trend_pipeline.mjs --topic "Whipped Feta Honey" --brain backboard
 *   node scripts/trend_pipeline.mjs --interactive
 *   npm run pipeline
 */

import { fetchTrendSignals } from '../src/services/trendCollector.js';
...
if (args.includes('--help') || args.includes('-h')) {
  console.log(`
${C.bold}${C.cyan}MiseEnVue Trend Intelligence Pipeline CLI${C.reset}
Identify trends & posts across Instagram, TikTok, Facebook, and Influencers,
synthesize them with an AI Brain (Gemini/Backboard), and strategize content.
...
${C.bold}OPTIONS:${C.reset}
  --topic <string>    The food, hospitality, or viral trend to analyze
                      (Default: "Crispy Falafel & Mezze")
  --brain <engine>    AI Brain engine: 'gemini' or 'backboard'
                      (Default: 'gemini')
...
async function promptInteractive() {
  ...
  console.log("  1. Crispy Falafel & Mezze");
  console.log("  2. Whipped Feta & Hot Honey Drizzle");
  console.log("  3. Lacy Edge Smash Burgers & Dipping Jus");
  console.log("  4. Cold Cloud Foam Matcha");
  console.log("  5. Custom topic...\n");

  const topicChoice = await question(`${C.bold}Select topic (1-5, or press Enter for default): ${C.reset}`);
  let topic = "Crispy Falafel & Mezze";
  if (topicChoice === "1") topic = "Crispy Falafel & Mezze";
  else if (topicChoice === "2") topic = "Whipped Feta & Hot Honey Drizzle";
  else if (topicChoice === "3") topic = "Lacy Edge Smash Burgers & Dipping Jus";
  else if (topicChoice === "4") topic = "Cold Cloud Foam Matcha";
...
async function main() {
  ...
  console.log("\n" + "=".repeat(76));
  console.log(`  ${C.bold}${C.cyan}🚀 SOCIAL TREND INTELLIGENCE PIPELINE${C.reset}`);
  ...
  console.log(`  ${C.bold}AI Brain:${C.reset}     ${brain === 'gemini' ? C.green + 'Google Gemini 2.5 Flash' : C.blue + 'Backboard AI'}${C.reset}`);
  ...
  // STAGE 1: IDENTIFY
  // -------------------------------------------------------------------------
  console.log(`\n${C.bgCyan} [STAGE 1: IDENTIFY] ${C.reset} ${C.bold}Collecting trend signals & social posts...${C.reset}`);
  const posts = fetchTrendSignals(topic);
  
  console.log(`\n  ${C.green}✓ Ingested ${posts.length} high-velocity posts across 4 channels:${C.reset}\n`);
  ...
  for (const p of posts) {
    ...
    const authorStr = p.authorName.slice(0, 22).padEnd(24);
    const viewsStr = (p.views >= 1e6 ? (p.views/1e6).toFixed(1)+'M' : (p.views/1e3).toFixed(0)+'K').padEnd(10);
    const erStr = p.engagementRate.padEnd(8);
    const hookStr = p.hookType.slice(0, 24);
  ...
  const reportContent = `# Trend-to-Content Action Report: ${topic}
- **Generated At:** ${new Date().toISOString()}
- **AI Brain:** ${brain === 'gemini' ? 'Google Gemini 2.5 Flash' : 'Backboard AI'}
- **Analyzed Signals:** ${posts.length} posts across Instagram, TikTok, Facebook, and Influencers
